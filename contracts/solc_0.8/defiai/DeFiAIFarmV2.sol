...
contract DeFiAIFarmV2 is Ownable {
    ...
    function getTotalBalance(address _user) 
    external 
    view 
    returns (uint256) 
    {
        ...
        external
    {
        ...
        require (IERC20(msg.sender).allowance(msg.sender, address(this)) >= _wantAmt);
            ...
            IDeFiAIMultiStrat(poolInfo.strat).deposit(msg.sender, _wantAmt, address(poolInfo.want));
        ...
        external
    {
        ...
        _wantAmt = IDeFiAIMultiStrat(poolInfo.strat).withdraw(msg.sender, _wantAmt, address(poolInfo.want));
